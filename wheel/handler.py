...
# Replace these with a setting in the message for use in Friday Gaming
EVENT_TITLE="Happy Hour"
EVENT_HOUR_DEFAULT=16
EVENT_DURATION=5
...
async def spin_wheel(ctx: commands.Context):
    ...
    await ctx.guild.create_scheduled_event(
        name=f"{EVENT_TITLE} - {result}",
        start_time=time,
        end_time=time.replace(hour=time.hour + EVENT_DURATION),
        location={result},
        description=f"Event Created by The Wheel - generated by {ctx.author.name}")
